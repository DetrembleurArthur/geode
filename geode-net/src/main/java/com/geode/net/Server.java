...
public class Server extends Thread implements Initializable
{
    ...
    public synchronized void waitChargeAvailability() throws InterruptedException
    {
            ...
            {
                logger.warn("wait for charge availability ", getServerInfos().getName());
                wait();
                logger.warn("free charge availability ", getServerInfos().getName());
            }
    }
    ...
    {
        logger.warn("remove protocol handler", getServerInfos().getName());
